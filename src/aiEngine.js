...
const ATTRIBUTE_KEYWORDS = {
  str: ['тяжелая', 'атлетика', 'зал', 'жим', 'штанга', 'гантели', 'отжимания', 'подтягивания', 'приседания', 'мышцы', 'силовая', 'пресс', 'турник', 'брусья', 'бокс', 'борьба', 'карате', 'ММА', 'мма', 'единоборств', 'кроссфит', 'воркаут', 'тяга', 'жим лёжа', 'армрестлинг'],
  end: ['бег', 'пробежал', 'велосипед', 'кардио', 'бассейн', 'плавание', 'лыжи', 'ходьба', 'дорожка', 'шагов', 'дистанция', 'темп', 'велотренажер', 'эллипс', 'скакалка', 'интервал', 'табата', 'скандинавская ходьба'],
  agi: ['растяжка', 'йога', 'пилатес', 'гибкость', 'шпагат', 'разминка', 'суставы', 'гимнастика', 'стретчинг', 'плиометрика', 'танцы', 'коньки', 'сноуборд', 'скалолаз', 'акробатика', 'баланс', 'координац']
};

const HP_KEYWORDS = ['сон', 'выспался', 'чистое питание', 'вода', 'овощи', 'белок', 'калории', 'отдых', 'медитация', 'витамин', 'массаж', 'баня', 'сауна', 'свежий воздух', 'прогулка', 'релакс', 'здоров'];
...
const NEGATIVE_KEYWORDS = [
  ...
  'энергетик', 'снэк', 'дошик', 'дошир', 'лапша быстрого', 'роллтон', 'ред булл',
  'жирное', 'майонез', 'кетчуп', 'сухарик', 'нагетс', 'шаурма', 'хот-дог'
];
...
export const aiEngine = {
  async analyzeActivity(text, apiKey, charState, imageBase64 = null) {
    // Offline check: skip API call if no network
    ...
    if (apiKey && apiKey.trim() && isOnline) {
      try {
        ...
        console.warn("Gemini API failed, falling back to local simulator", e);
